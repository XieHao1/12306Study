...
import org.index12306.cache.SeatMarginCacheLoader;
import org.index12306.constant.Index12306Constant;
import org.index12306.constant.RedisKeyConstant;
import org.index12306.dto.domain.PurchaseTicketPassengerDetailDTO;
import org.index12306.dto.domain.RouteDTO;
import org.index12306.dto.domain.SeatClassDTO;
import org.index12306.dto.domain.TicketListDTO;
import org.index12306.dto.req.*;
import org.index12306.dto.resp.RefundTicketRespDTO;
import org.index12306.dto.resp.TicketOrderDetailRespDTO;
import org.index12306.dto.resp.TicketPageQueryRespDTO;
import org.index12306.dto.resp.TicketPurchaseRespDTO;
import org.index12306.entity.*;
import org.index12306.enums.*;
...
import org.index12306.framework.starter.cache.toolkit.CacheUtil;
import org.index12306.framework.starter.common.toolkit.BeanUtil;
import org.index12306.framework.starter.convention.exception.ServiceException;
import org.index12306.framework.starter.convention.result.Result;
import org.index12306.framework.starter.designpattern.chain.AbstractChainContext;
import org.index12306.frameworks.starter.user.core.UserContext;
import org.index12306.handler.dto.TrainPurchaseTicketRespDTO;
import org.index12306.handler.select.TrainSeatTypeSelector;
import org.index12306.handler.tokenbucket.TicketAvailabilityTokenBucket;
import org.index12306.mapper.*;
import org.index12306.remote.PayRemoteService;
import org.index12306.remote.TicketOrderRemoteService;
import org.index12306.remote.dto.*;
...
import org.index12306.toolkit.DateUtil;
import org.index12306.toolkit.TimeStringComparator;
...
public class TicketServiceImpl extends ServiceImpl<TicketMapper, TicketDO> implements TicketService, CommandLineRunner {
    ...
    public TicketPageQueryRespDTO pageListTicketQueryV1(TicketPageQueryReqDTO requestParam) {
        ...
        List<Object> stationDetails = stringRedisTemplate.opsForHash()
                .multiGet(RedisKeyConstant.REGION_TRAIN_STATION_MAPPING, Lists.newArrayList(requestParam.getFromStation(), requestParam.getToStation()));
        long count = stationDetails.stream().filter(Objects::isNull).count();
        if (count > 0) {
            RLock lock = redissonClient.getLock(RedisKeyConstant.LOCK_REGION_TRAIN_STATION_MAPPING);
            lock.lock();
            try {
                ...
                if (count > 0) {
                    ...
                    stringRedisTemplate.opsForHash().putAll(RedisKeyConstant.REGION_TRAIN_STATION_MAPPING, regionTrainStationMap);
        ...
        String buildRegionTrainStationHashKey = String.format(RedisKeyConstant.REGION_TRAIN_STATION, stationDetails.get(0), stationDetails.get(1));
        Map<Object, Object> regionTrainStationAllMap = stringRedisTemplate.opsForHash().entries(buildRegionTrainStationHashKey);
        if (MapUtil.isEmpty(regionTrainStationAllMap)) {
            RLock lock = redissonClient.getLock(RedisKeyConstant.LOCK_REGION_TRAIN_STATION);
            lock.lock();
            try {
                ...
                if (MapUtil.isEmpty(regionTrainStationAllMap)) {
                    ...
                    for (TrainStationRelationDO each : trainStationRelationList) {
                        TrainDO trainDO = distributedCache.safeGet(
                                RedisKeyConstant.TRAIN_INFO + each.getTrainId(),
                                TrainDO.class,
        ...
        for (TicketListDTO each : seatResults) {
            String trainStationPriceStr = distributedCache.safeGet(
                    String.format(RedisKeyConstant.TRAIN_STATION_PRICE, each.getTrainId(), each.getDeparture(), each.getArrival()),
                    String.class,
                    () -> {
    ...
    public TicketPageQueryRespDTO pageListTicketQueryV2(TicketPageQueryReqDTO requestParam) {
        ...
        List<String> trainStationPriceKeys = seatResults.stream()
                .map(each -> String.format(cacheRedisPrefix + RedisKeyConstant.TRAIN_STATION_PRICE, each.getTrainId(), each.getDeparture(), each.getArrival()))
                .toList();
        ...
        for (Object each : trainStationPriceObjs) {
            ...
            for (TrainStationPriceDO item : trainStationPriceList) {
                String trainStationRemainingKey = cacheRedisPrefix + RedisKeyConstant.TRAIN_STATION_REMAINING_TICKET + StrUtil.join("_", item.getTrainId(), item.getDeparture(), item.getArrival());
    ...
    public TicketPurchaseRespDTO purchaseTicketsV2(PurchaseTicketReqDTO requestParam) {
        ...
        seatTypeMap.forEach((searType, count) -> {
            String lockKey = environment.resolvePlaceholders(String.format(RedisKeyConstant.LOCK_PURCHASE_TICKETS_V2, requestParam.getTrainId(), searType));
    ...
    public void cancelTicketOrder(CancelTicketOrderReqDTO requestParam) {
        ...
        if (cancelOrderResult.isSuccess() && !StrUtil.equals(ticketAvailabilityCacheUpdateType, "binlog")) {
            Result<org.index12306.remote.dto.TicketOrderDetailRespDTO> ticketOrderDetailResult = ticketOrderRemoteService.queryTicketOrderByOrderSn(requestParam.getOrderSn());
            org.index12306.remote.dto.TicketOrderDetailRespDTO ticketOrderDetail = ticketOrderDetailResult.getData();
                ...
                routeDTOList.forEach(each -> {
                    ...
                    seatTypeMap.forEach((seatType, ticketOrderPassengerDetailRespDTOList) -> {
                        stringRedisTemplate.opsForHash()
                                .increment(RedisKeyConstant.TRAIN_STATION_REMAINING_TICKET + keySuffix, String.valueOf(seatType), ticketOrderPassengerDetailRespDTOList.size());
                    });
                });
            } catch (Throwable ex) {
    ...
    public RefundTicketRespDTO commonTicketRefund(RefundTicketReqDTO requestParam) {
        ...
        Result<org.index12306.remote.dto.TicketOrderDetailRespDTO> orderDetailRespDTOResult = ticketOrderRemoteService.queryTicketOrderByOrderSn(requestParam.getOrderSn());
        if (!orderDetailRespDTOResult.isSuccess() && Objects.isNull(orderDetailRespDTOResult.getData())) {
            throw new ServiceException("车票订单不存在");
        }
        org.index12306.remote.dto.TicketOrderDetailRespDTO ticketOrderDetailRespDTO = orderDetailRespDTOResult.getData();
